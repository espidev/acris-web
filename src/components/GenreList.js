...
import {getGenres} from "../api/collection";
...
class GenreList extends React.Component {
    ...
    componentDidMount() {
        ...
        getGenres(this.props.collection.id)
            .then(res => {
                if (this.componentMounted) {
                    this.setState({loading: false, genres: res.data});
                }
            })
            .catch(err => {
                if (this.componentMounted) {
                    this.setState({
                        loading: false,
                        alerts: addAlert(this.state.alerts, 'Issue fetching genres list.', 'danger', getUniqueId()),
                    });
                    console.log("Issue fetching genres list: " + err);
                }
            });
    }
    ...
    render() {
        ...
        const PageHeader = () => (
            <React.Fragment>
                ...
                <PageSection>
                    <TextContent>
                        <Text component={TextVariants.h1}>Genres</Text>
                    </TextContent>
                </PageSection>
            </React.Fragment>
        );
                ...
                <React.Fragment>
                    ...
                    <PageSection>
                        <EmptyState>
                            ...
                            <Title headingLevel="h2" size="lg">No genres</Title>
                            <EmptyStateBody>
                    ...
                    <PageSection>
                        <CardLayout>
                            {this.state.genres.map((genre, key) => {
                                return (
                                    <CardComponent width="15em" key={key} url={'/collection/' + this.props.collection.id + '/genre/' + genre.id}>
                                        ...
                                        <CardHeader style={{textAlign: 'center'}}>{genre.name}</CardHeader>
                                    </CardComponent>
                                );
                            })}
                        </CardLayout>
